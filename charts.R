...
pie_data <- data |> 
  filter(is_hit == TRUE) |>              # Keep only rows where a Hit occurred
...
ggplot(pie_data, aes(x = "", y = n, fill = first_pitch_result)) +
  ...
  labs(
    title = "Hits",
    subtitle = "What was the First Pitch for every hit recorded?",
...
# 2. Prepare the Data: Filter for HITS only
pie_data <- data |> 
  filter(is_walk == TRUE) |>              # Keep only rows where a Hit occurred
  filter(first_pitch_result %in% c("Strike", "Ball")) |> # Remove "Other"
  count(first_pitch_result) |>           # Count how many were Strikes vs Balls
  mutate(
    percentage = n / sum(n),             # Calculate %
...
ggplot(pie_data, aes(x = "", y = n, fill = first_pitch_result)) +
  ...
  labs(
    title = "Walks",
    subtitle = "What was the First Pitch for every walk recorded?",
    fill = "First Pitch Result", # Legend Title
    x = NULL, y = NULL
  ) +
